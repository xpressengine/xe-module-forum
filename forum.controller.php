...
class forumController extends forum {
	...
	function procForumInsertDocument()
	{
		...
		if($this->module_info->module != "forum") 
		...
		if(Context::get('is_logged') && $logged_info->is_admin!='Y') 
			$obj->allow_comment='Y';
		...
		else 
		{
			...
			if(!$insert_alias) 
				return new Object(-1,'thread title already exists');
	...
	function procForumBanUser()
	{
		...
		if($obj->ban_id)
		{
			$member_info = $oMemberModel->getMemberInfoByMemberSrl($obj->member_srl);
			if($member_info) 
				$output = executeQuery('member.insertDeniedID', $member_info);
		}
		if($obj->ban_ip) if($obj->ipaddress)
			$output = executeQuery('spamfilter.insertDeniedIP', $obj);
		if($obj->delete_user)
	...
	function triggerMemberMenu(&$obj)
	{
		...
		if($cur_module_info->module != 'forum')
			return;
